...
#define ORACLE_TOL_R            1e-2f  /* widened: density torsion actively desynchronizes phases */
#define ORACLE_TOL_VEL          1e-2f
#define ORACLE_DENSITY_MAX_FRAC 0.95f   /* no single shell should hold >95% of particles (collapse) */
    ...
    int N, int frame)
{
    ...
    if (orc->initialized) {
        ...
        /* Density: structural bounds check.
         * A healthy disk with spiral arms has a rotating density pattern —
         * the radial histogram shifts every frame, and that's correct.
         * Instead of checking for stasis, check that mass hasn't collapsed
         * into a single shell (>60% in one bin = gravitational collapse). */
        for (int s = 0; s < ORACLE_NUM_SHELLS; s++) {
            if (density_hist[s] > ORACLE_DENSITY_MAX_FRAC) pass_dens = 0;
        }

        /* Topo: L2 norm of frame-to-frame shift */
        float topo_l2 = 0;
        for (int i = 0; i < 5; i++) {
