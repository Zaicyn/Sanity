...
#define ISCO_R               1.0f    /* soft floor, no GR ISCO */
...
static void init_particles(ParticleState* ps, int N, unsigned int seed) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        ps->theta[i] = atan2f(z, x) + ((float)rand() / RAND_MAX) * 0.2f;

        /* Keplerian omega: inner particles faster, outer slower.
         * This gives real differential rotation that spreads theta
         * across the full ring, making the Viviani counter-rotation visible. */
        {
            float r_ref = 50.0f;
            float r_clamped = fmaxf(r_xz, 1.0f);
            ps->omega_nat[i] = 0.1f * powf(r_ref / r_clamped, 1.5f);
        }
    ...
    float* ax, float* ay, float* az)
{
    ...
    /* Per-particle fiber phase — each particle has its own position on
     * the Viviani curve. The 3θ harmonic winding produces different
     * tangent directions for different phases → counter-rotating shells. */
    float theta = theta_fiber;
...
static void physics_step(ParticleState* ps, float dt, float time) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        /* Viviani field force — uses per-particle fiber phase */
        float ax = 0, ay = 0, az = 0;
        float theta_fiber = ps->theta[i];
        apply_viviani_field(px, py, pz, r3d, inv_r3d, theta_fiber, &ax, &ay, &az);
