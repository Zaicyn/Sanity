...
static int rateDeviceSuitability(VkPhysicalDevice device, VkSurfaceKHR surface) {
    ...
    // Prefer NVIDIA for CUDA interop
...
void pickPhysicalDevice(VulkanContext& ctx) {
    ...
    std::cout << "[V20] Selected GPU: " << deviceProperties.deviceName << std::endl;
...
void createLogicalDevice(VulkanContext& ctx) {
    ...
    VkPhysicalDeviceFeatures deviceFeatures = {};
    deviceFeatures.fillModeNonSolid = VK_TRUE;  // For wireframe debugging
    deviceFeatures.largePoints = VK_TRUE;       // For particle rendering
    ...
    std::cout << "[V20] Logical device created" << std::endl;
}
