// v21_spirv.h — SPIRV-First Compute Runtime
// ===============================================
...
typedef struct {
    V21Kernel kernels[V21_MAX_KERNELS];
    int         count;
...
static inline void spirv_registry_init(V21Registry* reg, const char* backend) {
    reg->count = 0;
...
static inline int spirv_register_kernel(
    ...
    int push_size)
{
    if (reg->count >= V21_MAX_KERNELS) return -1;
...
// Find kernel by name
static inline int spirv_find_kernel(const V21Registry* reg, const char* name) {
