...
#define SEAM_STEP            (6.28318530718f / 32.0f)  /* 2π/32 = one ring position */

/* Squaragon V2 LUTs — CPU copies of siphon.comp ground truth.
 * Used by torus-aware init to set velocity direction from fiber phase. */
static const int CPU_FLOW_MODE[32] = {
    0, 1, 1, 1, 2, 0, 0, 2, 1, 2, 0, 0, 2, 1, 1, 1,
    0, 1, 1, 1, 2, 0, 0, 2, 1, 2, 0, 0, 2, 1, 1, 1
};

static const float CPU_TANGENT[32][3] = {
    { -1.000000f,  0.000000f, -0.000000f},
    { -0.138370f, -0.331496f, -0.933255f},
    {  0.120303f, -0.344944f, -0.930882f},
    {  0.408600f, -0.332813f, -0.849871f},
    {  0.857493f, -0.171499f, -0.485071f},
    {  0.941049f,  0.250189f,  0.227669f},
    {  0.666393f,  0.564428f,  0.487177f},
    {  0.386358f,  0.836993f,  0.387517f},
...
static void init_particles(ParticleState& ps, int N, unsigned int seed,
    ...
    int capacity = (capacity_override > 0) ? capacity_override : N;
    if (capacity < N) capacity = N;
    ps.N = N;
    ps.capacity = capacity;
    ps.pos_x = (float*)calloc(capacity, sizeof(float));
    ps.pos_y = (float*)calloc(capacity, sizeof(float));
    ps.pos_z = (float*)calloc(capacity, sizeof(float));
    ps.vel_x = (float*)calloc(capacity, sizeof(float));
    ps.vel_y = (float*)calloc(capacity, sizeof(float));
    ps.vel_z = (float*)calloc(capacity, sizeof(float));
    ps.pump_scale = (float*)calloc(capacity, sizeof(float));
    ps.pump_residual = (float*)calloc(capacity, sizeof(float));
    ps.pump_history = (float*)calloc(capacity, sizeof(float));
    ps.pump_state = (int*)calloc(capacity, sizeof(int));
    ps.theta = (float*)calloc(capacity, sizeof(float));
    ps.omega_nat = (float*)calloc(capacity, sizeof(float));
    ps.flags = (uint8_t*)calloc(capacity, sizeof(uint8_t));
    ps.topo_state = (uint8_t*)calloc(capacity, sizeof(uint8_t));
    ...
    for (int i = 0; i < n_generate; i++) {
        ...
        /* Torus-aware init: theta from orbital angle + seam step,
         * velocity direction from TANGENT[gen] LUT, magnitude Keplerian. */
        float orbital_angle = atan2f(z, x);  /* [-π, π] */
        p.theta = orbital_angle + SEAM_STEP;
        if (p.theta >= 6.28318530718f) p.theta -= 6.28318530718f;
        if (p.theta < 0.0f)           p.theta += 6.28318530718f;

        int gen = (int)(p.theta * (32.0f / 6.28318530718f)) & 31;

        float r_xz = sqrtf(x*x+z*z);
        float v_circ = sqrtf(BH_MASS / fmaxf(r_xz, 1.0f));
        p.vx = CPU_TANGENT[gen][0] * v_circ;
        p.vy = CPU_TANGENT[gen][1] * v_circ;
        p.vz = CPU_TANGENT[gen][2] * v_circ;

        /* Keplerian omega: inner particles faster, outer slower.
         * omega ∝ r^(-3/2) gives real differential rotation. */
        {
            float r_ref = 50.0f;
            float r_clamped = fmaxf(r_xz, 1.0f);
            p.omega_nat = 0.1f * powf(r_ref / r_clamped, 1.5f);
        }
        int axis = rand()%4; int sign_t = (rand()%2)?1:-1;
        p.topo_state = (uint8_t)(((sign_t>0)?1:2) << (axis*2));
        tmp[i] = p;
        ...
        int sort_gen = (int)(p.theta * (32.0f / 6.28318f)) & 31;
        uint32_t mode = FM[sort_gen];
...
/* ========================================================================
 * CPU-SIDE SPAWN — Squaragon cell cycle (384/432/496 thresholds)
 * ======================================================================== */

static int spawn_step(ParticleState& ps, PhysicsCompute& phys, VulkanContext& ctx) {
    /* Re-query VRAM to get actual free space right now */
    int vram_cap = queryVramMaxParticles(ctx.physicalDevice);
    int cap = (vram_cap < ps.capacity) ? vram_cap : ps.capacity;

    if (ps.N >= cap) return 0;

    /* Allocator thresholds on the VRAM-aware cap */
    int bias_eq  = (int)(cap * 0.75f);   /* 384/512 = equilibrium */
    int working  = (int)(cap * 0.844f);  /* 432/512 = slow down */
    int redline  = (int)(cap * 0.969f);  /* 496/512 = stop */

    if (ps.N >= redline) return 0;

    /* Exponential growth: every parent spawns, throttled only by thresholds */
    int budget;
    if (ps.N < bias_eq) {
        budget = ps.N;  /* double every step */
    } else if (ps.N < working) {
        float t = (float)(ps.N - bias_eq) / (float)(working - bias_eq);
        budget = (int)(ps.N * (1.0f - t * 0.8f));  /* 100% → 20% of N */
    } else {
        budget = ps.N / 100;  /* trickle: 1% growth */
    ...
    for (int i = 0; i < ps.N && spawned < budget; i++) {
        if (!(ps.flags[i] & 0x01)) continue;

        int slot = old_N + spawned;
        if (slot >= cap) break;

        /* Child theta = parent theta + one seam step */
        float theta_child = ps.theta[i] + SEAM_STEP;
        if (theta_child >= 6.28318530718f) theta_child -= 6.28318530718f;
        int child_gen = (int)(theta_child * (32.0f / 6.28318530718f)) & 31;

        /* Child position: small offset from parent */
        float px = ps.pos_x[i] + ((float)rand()/RAND_MAX - 0.5f) * 2.0f;
        float py = ps.pos_y[i] + ((float)rand()/RAND_MAX - 0.5f) * 0.5f;
        float pz = ps.pos_z[i] + ((float)rand()/RAND_MAX - 0.5f) * 2.0f;
        ...
        ps.pos_y[slot] = py;
        ps.pos_z[slot] = pz;
        ps.vel_x[slot] = CPU_TANGENT[child_gen][0] * v_circ;
        ps.vel_y[slot] = CPU_TANGENT[child_gen][1] * v_circ;
        ps.vel_z[slot] = CPU_TANGENT[child_gen][2] * v_circ;
        ps.theta[slot] = theta_child;
        float r_clamped = fmaxf(r_xz, 1.0f);
        ps.omega_nat[slot] = 0.1f * powf(50.0f / r_clamped, 1.5f);
        ps.flags[slot] = 0x01;
        ps.pump_scale[slot] = 1.0f;
    ...
    /* Partial upload: only the new slots, for each active SSBO */
    size_t f_off = (size_t)old_N * sizeof(float);
    size_t f_sz  = (size_t)spawned * sizeof(float);
    size_t u_off = (size_t)old_N * sizeof(uint32_t);
    size_t u_sz  = (size_t)spawned * sizeof(uint32_t);

    /* bindings: 0-5=pos/vel, 12=theta, 13=omega_nat, 14=flags */
    uploadToSSBO_offset(ctx, phys.soa_buffers[0],  ps.pos_x + old_N,     f_off, f_sz);
    uploadToSSBO_offset(ctx, phys.soa_buffers[1],  ps.pos_y + old_N,     f_off, f_sz);
    uploadToSSBO_offset(ctx, phys.soa_buffers[2],  ps.pos_z + old_N,     f_off, f_sz);
    uploadToSSBO_offset(ctx, phys.soa_buffers[3],  ps.vel_x + old_N,     f_off, f_sz);
    uploadToSSBO_offset(ctx, phys.soa_buffers[4],  ps.vel_y + old_N,     f_off, f_sz);
    uploadToSSBO_offset(ctx, phys.soa_buffers[5],  ps.vel_z + old_N,     f_off, f_sz);
    uploadToSSBO_offset(ctx, phys.soa_buffers[12], ps.theta + old_N,     f_off, f_sz);
    uploadToSSBO_offset(ctx, phys.soa_buffers[13], ps.omega_nat + old_N, f_off, f_sz);

    /* flags is uint8_t on CPU but uint32_t on GPU — need to widen */
    std::vector<uint32_t> flags32(spawned);
    for (int j = 0; j < spawned; j++)
        flags32[j] = ps.flags[old_N + j];
    uploadToSSBO_offset(ctx, phys.soa_buffers[14], flags32.data(), u_off, u_sz);
...
int main(int argc, char** argv) {
    ...
    /* Growth capacity: set after Vulkan init from 80% of free VRAM.
     * Placeholder here — overwritten below once we can query the GPU. */
    int growth_capacity = num_particles;
        ...
        if (use_gpu_physics) {
            ...
            if (gpu_ok) {
                ...
                initPhysicsCompute(gpuPhys, headlessCtx,
                    ...
                    num_particles, scatter_mode, particles.capacity);
    ...
    /* Keyboard: R cycles render mode (0=all, 1=alive, 2=nova, 3=crystal) */
    static int* s_render_mode_ptr = &render_mode;
    glfwSetKeyCallback(window, [](GLFWwindow*, int key, int, int action, int) {
        if (key == GLFW_KEY_R && action == GLFW_PRESS) {
            *s_render_mode_ptr = (*s_render_mode_ptr + 1) % 4;
            const char* names[] = {"ALL", "ALIVE only", "NOVA only", "CRYSTAL only"};
            printf("[render] Mode %d: %s\n", *s_render_mode_ptr, names[*s_render_mode_ptr]);
        }
    });
    ...
    /* Query VRAM and set growth capacity — 80% of free device-local memory.
     * The allocator's thresholds (75%/84%/97%) control fill rate within this. */
    if (use_gpu_physics) {
        int vram_cap = queryVramMaxParticles(vkCtx.physicalDevice);
        growth_capacity = vram_cap;
        if (growth_capacity < num_particles) growth_capacity = num_particles;

        /* Grow CPU arrays to VRAM capacity if needed */
        if (growth_capacity > particles.capacity) {
            auto grow_f = [&](float*& arr, int old_n) {
                float* tmp = (float*)calloc(growth_capacity, sizeof(float));
                memcpy(tmp, arr, (size_t)old_n * sizeof(float));
    ...
    if (use_gpu_physics) {
        initPhysicsCompute(gpuPhys, vkCtx,
            ...
            particles.theta, particles.omega_nat,
            particles.flags, particles.topo_state,
            particles.N, scatter_mode, particles.capacity);
    ...
    /* Async readback state: copy is issued in frame N's command buffer,
     * data is consumed in frame N+1 after the fence confirms completion.
     * Zero stalls — diagnostics overlap with GPU execution. */
    bool readback_pending = false;
    int readback_frame = 0;
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (!use_gpu_physics) {
            ...
            /* Readback is now async — copies are recorded into the GPU command
             * buffer (see below). We just mark that we want diagnostics next frame. */
            (void)0;  /* readback_pending set below after recordReadbackCopies */
        }

        /* (Async readback diagnostics processed after vkWaitForFences below) */
        if (false) {
        ...
        /* Draw: acquire → record → submit → present */
        {
            ...
            /* ---- Process async readback (GPU is idle, staging buffer is safe) ----
             * Copy commands were recorded in frame N-1's command buffer.
             * The fence we just waited on confirms they completed.
             * Process diagnostics here — CPU work overlaps with NEXT frame's GPU. */
            if (readback_pending) {
                readback_pending = false;
                consumeReadback(gpuPhys,
                    rb_px, rb_py, rb_pz,
                    rb_vx, rb_vy, rb_vz,
                ...
                printf("[oracle] frame=%d count=%d -> %s  flags0=0x%02x\n",
                       readback_frame, oracle_count, result, (unsigned)rb_flags[0]);

                /* Pump state is a stub — just zero it */
                readbackPumpStateSample(gpuPhys, vkCtx, rb_pump_state, oracle_count);
                int state_hist[8] = {0};
                for (int i = 0; i < oracle_count; i++) {
                    int s = rb_pump_state[i];
            ...
            if (use_gpu_physics) {
                ...
                /* Async readback: record copy commands every 500 frames.
                 * Data is consumed NEXT frame after vkWaitForFences confirms completion. */
                if (frame > 0 && frame % 500 == 0) {
                    recordReadbackCopies(gpuPhys, cmd, oracle_count);
