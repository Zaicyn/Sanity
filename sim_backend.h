// sim_backend.h — Abstract Compute Backend Interface
// ===================================================
...
//
// Usage:
//   SimBackend* backend = create_vulkan_backend(vkCtx);
//   // or: SimBackend* backend = create_cuda_backend();
//
...
enum SimCopyDir {
    SIM_COPY_HOST_TO_DEVICE = 1,
    SIM_COPY_DEVICE_TO_HOST = 2,
    SIM_COPY_DEVICE_TO_DEVICE = 3
};
...
typedef struct SimBackend {
    ...
    void  (*copy)(void* dst, const void* src, size_t size, enum SimCopyDir dir);
...
#define SIM_LAUNCH_1D(backend, name, n, block_size, args, nargs) \
    (backend)->launch((name), \
        ((n) + (block_size) - 1) / (block_size), 1, 1, \
        (block_size), 1, 1, \
        (args), (nargs), NULL)

#define SIM_LAUNCH_1D_STREAM(backend, name, n, block_size, args, nargs, stream) \
    (backend)->launch((name), \
        ((n) + (block_size) - 1) / (block_size), 1, 1, \
        (block_size), 1, 1, \
        (args), (nargs), (stream))
